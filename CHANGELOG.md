...
- **`FORCE_COLOR` env var** is honored alongside `NO_COLOR`. Setting
  it (to any non-empty, non-`0` value) forces ANSI escape passthrough
  even when stdout is not a TTY — useful when piping findjar through
  pagers (`less -R`), redirecting to a file you intend to re-display,
  or capturing for terminal-rendering tools (asciinema, svg-term-cli,
  freeze). `-m` and `NO_COLOR` still win.

### Documentation

- README: rewritten as a tool pitch with badges, a colored hero SVG
  demo, a "Why findjar?" comparison table (vs grep, zgrep, ripgrep,
  IDE search), and worked output examples for every feature in the
  "Usage at a glance" section.
- `doc/HELP.txt` and `doc/EXAMPLES.txt` snapshots committed; the
  README links to these so GitHub browsers can read them without
  installing. Regenerated by `clj -T:build snapshot-docs`.
- New `clj -T:build regen-docs` task and pre-release docs checklist
  in `doc/RELEASING.md`.
- Man page broadened to mention tar archives, `-w` / `-v` /
  `--completions`, `FORCE_COLOR`, `FINDJAR_DEBUG`.

## 1.0.140 — Tier 3 features merged
